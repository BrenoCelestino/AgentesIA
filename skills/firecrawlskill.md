...
### Observação para Agentes (Uso de CLI em Terminais e Fallback Nativo)

- Agentes que executam buscas em tempo real devem **usar o Terminal/CLI** (`firecrawl`) como primeira opção para acessar a web durante a sessão, devido à sua capacidade avançada de raspagem.
- **MUITO IMPORTANTE (Fallback):** Caso o `firecrawl search` ou o scraping falhem (devido a timeout, erro 403, falha de parsing), você **DEVE IMEDIATAMENTE** recorrer à ferramenta nativa `search_web` integrada. Ela é rápida e precisa, permitindo que você obtenha os resultados de busca e resumos de forma resiliente sem bloquear o fluxo da tarefa.
- Use `firecrawl/cli` e `firecrawl-search` quando precisar descobrir páginas, e `firecrawl-scrape` quando já tiver URLs.
- Consulte mais abaixo exemplos de comandos e práticas de autenticação antes de rodar operações que consomem créditos.
...
Use this API key for this onboarding session (configure inside .env):
